@section('title', 'Dashboard - Yug Parivartan Samiti')
@extends('layout.admin')
@section('content')

    <div class="content-page">
        <div class="content">

            <!-- Start Content-->
            <div class="container-fluid">

                <div class="py-3 d-flex align-items-sm-center flex-sm-row flex-column">
                    <div class="flex-grow-1">
                        <h4 class="fs-18 fw-semibold m-0">Edit Project</h4>
                    </div>

                    <div class="text-end">
                        <ol class="breadcrumb m-0 py-0">
                            <li class="breadcrumb-item"><a href="/admin/dashboard">Dashboard</a></li>
                            <li class="breadcrumb-item active">Project</li>
                        </ol>
                    </div>
                </div>

                <!-- General Form -->
                <div class="row">
                    <div class="col-12">
                        <div class="card">

                            <div class="card-header">
                                <h5 class="card-title mb-0">Project Details</h5>
                            </div><!-- end card header -->

                            <div class="card-body">
                                <form method="post" enctype="multipart/form-data">
                                    @csrf
                                    <div class="row">
                                        <div class="col-lg-6">
                                            <div class="mb-3">
                                                <label for="simpleinput" class="form-label">Project Name </label>
                                                <input type="text" id="simpleinput" name="project_name"
                                                    value="{{$Project->project_name}}" class="form-control">
                                                @error('project_name')
                                                    <div class="text-danger mt-2">{{ $message }}</div>
                                                @enderror
                                            </div>
                                        </div>


                                        <div class="col-lg-6">
                                            <div class="mb-3">
                                                <label for="simpleinput" class="form-label">Date</label>
                                                <input type="date"  id="simpleinput" name="date"
                                                    value="{{$Project->date}}" class="form-control">
                                                @error('date')
                                                    <div class="text-danger mt-2">{{ $message }}</div>
                                                @enderror
                                            </div>
                                        </div>
                                        <div class="col-lg-6">
                                            <div class="mb-3">
                                                <label for="simpleinput" class="form-label">Loction</label>
                                                <input type="loction"  id="simpleinput" name="loction"
                                                    value="{{$Project->loction}}" class="form-control">
                                                @error('location')
                                                    <div class="text-danger mt-2">{{ $message }}</div>
                                                @enderror
                                            </div>
                                        </div>

                                      

                                        <div class="col-lg-6">
                                            <div class="mb-3">
                                                <label for="example-select" class="form-label">Select Category</label>
                                                <select name="category" class="form-select">
                                                    @foreach ($categories as $category)
                                                        <option value="{{ $category->id }}"
                                                            {{ old('category', $Project->category_id) == $category->id ? 'selected' : '' }}>
                                                            {{ $category->title }}
                                                        </option>
                                                    @endforeach
                                                </select> @error('category')
                                                    <div class="text-danger mt-2">{{ $message }}</div>
                                                @enderror
                                            </div>
                                        </div>
                                        <div class="col-lg-6">
                                            <div class="mb-3">
                                                <label for="image" class="form-label">Post Image</label>
                                                <input type="file" accept="image/*" id="image" name="image" class="form-control">
                                                @error('image')
                                                    <div class="text-danger mt-2">{{ $message }}</div>
                                                @enderror
    
                                                <!-- Show current image -->
                                                @if($Project->image)
                                                    <div class="mt-2">
                                                        <img src="{{ asset('Project-image/' . $Project->image) }}" alt="Image" style="width: 100px;">
                                                    </div>
                                                @endif
                                            </div>
                                        </div>
<div class="col-12">
    <h4 class="bg-light p-3">Project Summery</h4>
</div>

                                        <div class="col-lg-12">
                                            <div class="mb-3">
                                                <label for="l_desc" class="form-label">Short Description</label>
                                                <textarea name="s_description" class="form-control" cols="30" rows="3">{{$Project->s_description}}</textarea>

                                                @error('s_description')
                                                    <div class="text-danger mt-2">{{ $message }}</div>
                                                @enderror

                                            </div>
                                        </div>


                                        <div class="col-lg-12">
                                            <div class="mb-3">
                                                <label for="l_desc" class="form-label">Description</label>
                                                <textarea name="l_description" id="summernote" class="form-control" cols="30" rows="3">{{$Project->l_description}}</textarea>
                                            </div>
                                        </div>

                                        <div class="col-12">
                                            <h4 class="bg-light p-3">Project Challenges</h4>
                                        </div>

                                      
                                        <div class="col-lg-12">
                                            <div class="mb-3">
                                                <label for="l_desc" class="form-label">Short Description</label>
                                                <textarea name="s_description1" class="form-control" cols="30" rows="3">{{$Project->s_description1}}</textarea>

                                                @error('s_description1')
                                                    <div class="text-danger mt-2">{{ $message }}</div>
                                                @enderror

                                            </div>
                                        </div>


                                        <div class="col-lg-12">
                                            <div class="mb-3">
                                                <label for="l_desc" class="form-label">Description</label>
                                                <textarea name="l_description1" id="summernote" class="form-control" cols="30" rows="3"> {{$Project->l_description1}}</textarea>
                                            </div>
                                        </div>

                                        <div class="col-lg-12">
                                            <div class="mb-3">
                                                <label for="image1" class="form-label">Post Image</label>
                                                <input type="file" accept="image/*" id="image1" name="image1" class="form-control">
                                                @error('image1')
                                                    <div class="text-danger mt-2">{{ $message }}</div>
                                                @enderror
    
                                                <!-- Show current image1 -->
                                                @if($Project->image1)
                                                    <div class="mt-2">
                                                        <img src="{{ asset('Project-Challenges-image/' . $Project->image1) }}" alt="Image" style="width: 100px;">
                                                    </div>
                                                @endif
                                            </div>
                                        </div>
                                        <div class="col-12">
                                            <h4 class="bg-light p-3">Project Mission</h4>
                                        </div>


                                        <div class="col-lg-12">
                                            <div class="mb-3">
                                                <label for="l_desc" class="form-label">Short Description</label>
                                                <textarea name="s_description2" class="form-control" cols="30" rows="3">{{$Project->s_description2}}</textarea>

                                                @error('s_description2')
                                                    <div class="text-danger mt-2">{{ $message }}</div>
                                                @enderror

                                            </div>
                                        </div>


                                        <div class="col-lg-12">
                                            <div class="mb-3">
                                                <label for="l_desc" class="form-label">Description</label>
                                                <textarea name="l_description2" id="summernote" class="form-control" cols="30" rows="3">{{$Project->l_description2}}</textarea>
                                            </div>
                                        </div>
                                       

                                      

                                        <div class="col-12">
                                            <h4 class="bg-light p-3">Seo Details</h4>
                                        </div>

                                        <div class="col-lg-12">
                                            <div class="mb-3">
                                                <label for="simpleinput" class="form-label">Meta Title</label>
                                                <input type="text" id="simpleinput" name="meta_title"
                                                    value="{{$Project->meta_title}}" class="form-control">
                                                @error('meta_title')
                                                    <div class="text-danger mt-2">{{ $message }}</div>
                                                @enderror
                                            </div>
                                        </div>

                                        <div class="col-lg-12">
                                            <div class="mb-3">
                                                <label for="simpleinput" class="form-label">Meta Tags</label>
                                                <input type="text" id="simpleinput" name="meta_tags"
                                                    value="{{$Project->meta_tags}}" class="form-control">
                                                @error('meta_tags')
                                                    <div class="text-danger mt-2">{{ $message }}</div>
                                                @enderror
                                            </div>
                                        </div>


                                        <div class="col-lg-12">
                                            <div class="mb-3">
                                                <label for="l_desc" class="form-label">Meta Description</label>
                                                <textarea name="meta_description" class="form-control" cols="30" rows="3">{{$Project->meta_description}}</textarea>
                                                @error('meta_description')
                                                    <div class="text-danger mt-2">{{ $message }}</div>
                                                @enderror
                                            </div>
                                        </div>


                                        <div class="col-lg-12">
                                            <div class="mb-3">
                                                <label for="simpleinput" class="form-label">Status</label>
                                                <div class="d-flex">
                                                    <!-- Active Option -->
                                                    <div class="form-check">
                                                        <input 
                                                            class="form-check-input" 
                                                            type="radio" 
                                                            value="Y" 
                                                            name="status" 
                                                            id="gridRadios1" 
                                                            {{ old('status', $Project->status) == 'Y' ? 'checked' : '' }}>
                                                        <label class="form-check-label" for="gridRadios1">
                                                            Active
                                                        </label>
                                                    </div>
                                        
                                                    <!-- Inactive Option -->
                                                    <div class="form-check ms-3">
                                                        <input 
                                                            class="form-check-input" 
                                                            type="radio" 
                                                            value="N" 
                                                            name="status" 
                                                            id="gridRadios2" 
                                                            {{ old('status', $Project->status) == 'N' ? 'checked' : '' }}>
                                                        <label class="form-check-label" for="gridRadios2">
                                                            Inactive
                                                        </label>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>

                                        <div class="col-lg-12">
                                            <div class="mb-3">
                                                <button type="submit" class="btn btn-primary">Submit</button>
                                            </div>
                                        </div>

                                    </div>
                                </form>
                            </div>

                        </div>
                    </div>
                </div>

            @section('header')
                <link href="https://cdn.jsdelivr.net/npm/summernote@0.9.0/dist/summernote-lite.min.css" rel="stylesheet">
            @endsection
            @section('footer')


                <script src="https://cdn.jsdelivr.net/npm/summernote@0.9.0/dist/summernote-lite.min.js"></script>


                {{-- <script src="https://cdn.jsdelivr.net/npm/summernote@0.8.18/dist/summernote-bs4.min.js"></script> --}}

                <script>
                    $('#summernote').summernote({
                        placeholder: '',
                        tabsize: 2,
                        height: 300,
                        toolbar: [
                            ['style', ['style']],
                            ['font', ['bold', 'underline', 'clear']],
                            ['color', ['color']],
                            ['fontsize', ['fontsize']],
                            ['para', ['ul', 'ol', 'paragraph']],
                            ['table', ['table']],
                            ['insert', ['link', 'picture', 'video']],
                            ['view', ['fullscreen', 'codeview', 'help']]
                        ],
                        fontSizes: ['8', '10', '12', '14', '16', '18', '20', '24', '28', '36', '48', '64', '82',
                            '150'
                        ], // Custom font sizes
                    });
                </script>
            @endsection


        </div> <!-- container-fluid -->

    </div> <!-- content -->


</div>

@endsection
